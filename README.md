# 5. Потоковая обработка и работа с real-time данными
...
### Grafana до оптимизаций

![](./assets/grafana_non_opt.png)

### Grafana после оптимизаций (кэшированная ONNX модель, настройки параллелизма)

![](./assets/grafana_opt.JPG)

- Тест метрик [measure_inference_metrics](./model-server/measure_inference_metrics.py)
...
```json
{
  "metrics": {
    ...
    "p95_ms": 19.66,
    "p99_ms": 21.7,
    "avg_latency_ms": 18.84,
    "min_latency_ms": 7.95,
    "max_latency_ms": 25.26,
    "error_rate": 0.0,
    "error_count": 0,
...
✅ Требование p99 < 30 мс
✅ Сократилось использование RAM (было 200-250 МБ, стало 70-80 МБ)

### В prometheus настроены алерты

![](./assets/alerts.JPG)

### При нагрузочном тестировании (x2 запросов)

- Задержка чуть увеличилась, но все еще в пределах требования p99 < 30 мс

![](./assets/grafana_stress.JPG)
